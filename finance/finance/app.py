...
def index():
    ...
    for stock in transactions_db:
        ...
        current_worth += stock["total_price"]

    print(current_worth)
    return render_template("index.html", database=transactions_db, cash=cash, total=current_worth)
...
def buy():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return apology("Enter Proper number of Shares")
...
def sell():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, date) VALUES (?, ?, ?, ?, ?)",
                   user_id, stock["symbol"], int((-1)*shares), stock["price"], date)
